...
def handler(request):
    ...
    try:
        ...
        try:
            ...
            # Verificar límite de Vercel (4.5 MB)
            if final_size > 4.5 * 1024 * 1024:
                logger.warning(f"[WARNING] File too large for Vercel ({final_size / 1024 / 1024:.2f} MB > 4.5 MB)")
                logger.warning("[WARNING] Returning stream URL instead")
                # Retornar la URL original para que el cliente descargue directamente
                return {
                    "statusCode": 413,  # Payload Too Large
                    "body": json.dumps({
                        "error": "File too large",
                        "streamUrl": stream_url,
                        "message": "Download directly from stream"
                    }),
                    "headers": {
                        "Content-Type": "application/json",
                        "Access-Control-Allow-Origin": "*",
                        "X-Metadata-Added": "false"
                    }
                }
            ...
            # Retornar binario directo (sin base64)
            return {
                "statusCode": 200,
                "body": file_data,  # Binario directo
