#!/usr/bin/env python3
"""
Download Server with Metadata Tagging
Procesa descargas de música agregando metadatos con mutagen
"""
...
class DownloadHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/api/download':
            try:
                ...
                # Detectar formato por Content-Type o extensión
    ...
    def log_message(self, format, *args):
        ...
        return  # Silenciar logs HTTP estándar

def run_server(port=8001):
    server_address = ('', port)
    httpd = HTTPServer(server_address, DownloadHandler)
    print(f"🎵 Download Server started on http://localhost:{port}")
    print(f"📦 Ready to process downloads with metadata")
    print(f"🎨 Supports FLAC and M4A formats")
